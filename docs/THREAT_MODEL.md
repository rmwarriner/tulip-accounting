...
| **High (integrity, mixed confidentiality)** | `audit_log.before_snapshot` / `audit_log.after_snapshot` | unencrypted SQLite (JSON) | 2 columns | **Integrity is High** (single `AuditLogWriter` chokepoint + the BEFORE UPDATE/DELETE triggers landed in #333, no other mutator path). **Confidentiality inherits the highest field tier of the snapshot's content** — `description_rectified` rows embed Critical free-text, `profile_updated` rows embed High-PII emails, etc. Treat as Critical-when-populated. User-supplied `transactions.description` lands here verbatim (audit L-21, #352) — the snapshot is the structural record of the rectification, not a sanitised view. Nulled on user erasure via the redaction pass (#235). **Retention: policy-driven tiers** — see ARCHITECTURE §7.2; default 7y for ledger mutations, 90d for auth events, 30d for AI consent / capability rows (#245). |
...
| **Medium (behavioural inference)** | `sinking_funds.target_date`, `sinking_funds.target_amount` | unencrypted SQLite | 2 columns | Discloses a *known future financial event* — a planned major purchase, tuition payment, down payment, etc. On its own a date + amount is innocuous; combined with `allocation_pools.name` ("New car", "Hawaii 2027") it's behavioural inference about household plans. Per audit L-18 (#352): treat as behavioural-inference exposure when surfacing in shared / exported contexts; tax-preparer journal exports with `?include_metadata=false` (#351) are still scoped to *transactions*, not sinking-fund plans. |
...
| **Per-field DEK wrapping** | `encrypt_field` uses the master key directly. One key compromise = all field-encrypted columns + every attachment file body. | Five field-encrypted columns today (`users.totp_secret_encrypted`, `accounts.external_account_number_encrypted`, `accounts.notes_encrypted`, `transactions.notes_encrypted`, `households.ai_keys_encrypted`) plus the attachment file body — broader scope than v1's single-column framing. Bounded blast radius for single-tenant local; promotion criterion is multi-tenant / cloud (Phase 10 re-audit). The `encrypt_field` API is stable across the future change. | ARCHITECTURE.md §7.4 (deferred from Phase 1). Wire-format version-byte + AAD binding (#338) shipped in Phase 8 — see below. |
| ~~**AEAD AAD binding + version-byte wire format**~~ | **Shipped — Phase 8 Wave-2 (#338).** `encrypt_field` now writes `0x02 \|\| nonce \|\| ct + tag` with caller-supplied AAD; `decrypt_field` dispatches on the leading byte, accepting v1 legacy (no AAD) and v2 (`field_aad(table, column, household_id, row_id)` per row). Cross-row / cross-column / cross-household ciphertext-swap by a DB-write attacker now fails AEAD authentication. Attachments use `attachments:content:{sha256}` AAD so content-addressed dedup keeps working. The wrap migration prefixes every existing v1 blob with `0x01` so dispatch is unambiguous. | Re-promoted from deferral by the deep security audit (M-1 + M-6). |
...
- **Importers handle untrusted input.** OFX / QIF / CSV parsers all run under explicit size + content-type guards in `tulip_api.routers.imports.upload_import`:
  ...
  - **OFX**: `ofxtools` (chosen over `ofxparse` for active maintenance). The parser uses a regex tag tokeniser, **not** an XML parser, so no XML processor is invoked at parse time and XXE is structurally impossible. (Earlier revisions of this document credited `defusedxml`; that was incorrect — `defusedxml` is not on the parse path.) Behaviour locked by `packages/tulip-importers/tests/test_ofx_security.py` which asserts `<!DOCTYPE>` / `<!ENTITY>` payloads do not trigger external-entity resolution.
