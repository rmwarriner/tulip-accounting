...
- **Bus-factor / successor recovery** is documented operator-facing in [`docs/RECOVERY.md`](RECOVERY.md). The Recovery Packet model assumes the master key, JWT secret, and a recent encrypted backup are stored outside the host; without them, the field-encrypted columns are cryptographic noise.
